...
gmx pdb2gmx -f test.gro
# sed -i "10,\$ { /^$filename/s/.*/$filename              $copies_small/ }" "topol.top" # adding the correct number of malodour molecules to the topology file
sed -i "10,\$ { /^Other/s/.*/PET              3/ }" "topol.top" # Adding correct number of PET to the topology file
sed -i "30,\$ { /^PET/s/.*/PET              $copies/ }" "topol.top" # Adding correct number of PET to the topology file
((copies--))

# Edit box conf to fit all molecules
gmx editconf -f conf.gro -o edited_conf.gro -c -d 1.0 -bt cubic -box $boxlen
