...
sed -i "1,\$ { /^energygrps              = MAL SOL/s/.*/energygrps              = $filename SOL}" "prod_md.mdp"
((copies--))

# Edit box conf to fit all molecules
gmx_mpi editconf -f "$filename.gro" -o edited_conf.gro -c -bt cubic -box $box_len
((copies_small--))
# Add PET
...
#gmx_mpi trjconv -s md_1.tpr -f md_1.trr -o md_1_noPBC.trr -pbc mol -center
